...
<body>
    ...
    <div class="c_con">
        <div class="bord">
            ...
            <ul class="r_lis_con">
                ...
                <li><b>按类型</b><a href="#" target="_blank" class="all">全部</a>
                    ...
                    | <a href="<?= Tv::getBaseUrl($params,'type_id').'&type_id='.$v['id']?>" ><?= Html::encode($v['name'])?></a>
                 <?php endforeach ?>
                </li>
                <li><b>按地区</b>  <a href="#" class="all">全部</a>
                    <?php foreach ($area as $v): ?>
                    | <a href="<?= Tv::getBaseUrl($params,'area_id').'&area_id='.$v['id']?>" ><?= Html::encode($v['area_name'])?></a>
                    <?php endforeach ?>
                </li>
                <li class="masted"><b>按时间</b>  <a href="#" class="all">全部</a>
                    <?php foreach ($year as $v): ?>
                    | <a href="<?= Tv::getBaseUrl($params,'year_id').'&year_id='.$v['id']?>"><?= Html::encode($v['years'])?></a>
                    <?php endforeach ?>
        ...
        <ul class="c_con_list">
            <?php foreach ($tv as $v): ?>
            <li class="x1">
                <a href="<?= Url::to(['tvc/index','id'=>$v['id']])?>" target="_blank"><img src="<?= Html::encode($v['img_url'])?>" /></a>
                <p><?= Html::encode($v['lanaguage_type'])?></p>
                <div><a href="<?= Url::to(['tvc/index','id'=>$v['id']])?>" target="_blank"><?= Html::encode($v['tv_name'])?></a> <img src="images/1.jpg" /> <em><?= Html::encode($v['score'])?></em></div>
            </li>
            <?php endforeach ?>
        </ul>
        <!--page-->
